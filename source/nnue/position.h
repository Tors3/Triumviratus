...
class Position {
   public:
    ...
    // --- Triumviratus bridge glue: minimal setup from a piece list (NO FEN parse,
    //     NO zobrist/checkers/movegen). Fills board/byTypeBB/byColorBB/pieceCount +
    //     a minimal StateInfo, enough for a full-refresh NNUE eval (inline accessors
    //     + Attacks). pcs[i]/sqs[i] in SF encoding; stm = side to move.
    void set_pieces(const Piece* pcs, const Square* sqs, int n, Color stm, StateInfo* si);
    // Flip/set side-to-move for the bridge's incremental do/undo (sideToMove is private).
    void set_side_to_move(Color c) { sideToMove = c; }
    ...
    // Exposed for the Triumviratus bridge's incremental do/undo (M3). In the master
    // this is private (only do_move/undo_move call it); the bridge replicates do_move
    // from its own move encoding, so it needs the same primitive.
