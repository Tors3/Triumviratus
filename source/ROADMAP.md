...
| 2026-06-04 | **CaptureHist bugfix** (`CaptureHist`/`CaptureHistDiv`) — capture-history era SEMPRE attiva ma ROTTA: caphist sommato grezzo (±7000 vs mvv_lva 100-605, schiacciava l'ordering ~10×) + malus alle catture fallite solo su cutoff-cattura (SF lo fa a OGNI cutoff). | **bugfix** | **REGRESSIONE da ~−15/−21 Elo RIMOSSA.** A/B sul default spedito (div1-raw) vs OFF = −21@178 / −12@344. Fix: `CaptureHistDiv=16` (scala SF) + malus su ogni cutoff → div16+fix vs OFF = **~0 (−0.3@1098, +0.8@458)** = neutro. Nuovo default = ON/div16/fix (recupera i ~+15-21 persi). Da confermare 8+0.08 (bugfix neg→neutro = safe) + portare nel repo col deploy. Indagine "+Elo da caphist" rinviata. |
