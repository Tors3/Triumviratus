...
int g_ttcut_bonus_scale = 111;   // /100 del td_stat_bonus(depth); spin "TTCutBonusScale"
...
bool g_eval_tt_write = false;  // [5.1 PROVATO E SCARTATO] cache static eval su MISS (SF search.cpp:830):
                               // su TT 1-via RADDOPPIA l'albero (2.12M->3.96M @bench14, NON node-neutral) =
                               // regressione. SF lo ha gratis col cluster 3-vie; noi no. Default OFF.
...
int g_pawn_hist_weight = 187;  // [4.1 BAKE 126->139]   // [3.7 BAKE 195->83] peso pawn-history /100 (200 = 2.0x come SF). Spin PawnHistoryWeight (granularita' /100, SPSA-friendly + permette frazioni <1).
...
int g_mainhist_weight = 93;  // [4.1 BAKE 122->168]   // [3.7 BAKE 209->131] /100 (100 = 1.0x; SF usa 2.0x=200)
...
int g_conthist_weight = 135;  // [4.1 BAKE 80->96]   // [3.7 BAKE 134->150] /100 (100 = 1.0x)
...
// ex-PV vengono ridotti MENO in LMR (reduction -= amount), perché un nodo già stato PV
// merita più attenzione. Il flag viene propagato negli store quando amount>0.
// REPARAMETRIZZATO (2026-06-24, lesson #14): da toggle bool a intero auto-spegnibile —
// 0=off (byte-identico), 1=−1ply (ex comportamento), 2=−2ply. Così il co-tune può
// dialarlo, e se resta dannoso (isolato era −2.6) lo SPSA lo riporta a 0 da solo.
int g_ttpv_amount = 1;
...
int g_hist_bonus_mult = 490;   // [4.1 BAKE 282->326] SPSA co-tune history/LMR @12+0.12 (+5-7 Elo)
...
bool g_tt_move_keep = true;          // TTMoveKeep (default off = byte-identico): conserva la TT move sui store fail-low senza mossa (SF). Alza ttrate ai cut-node. extern in tt.h.
...
// letto, questo declassa il RANKING di rimpiazzo). Letto da tt_victim() in tt.h.
...
int g_qs_capthist_scale = 86;   // % di td_stat_bonus(1)/malus — leva SPSA dedicata (bonus/malus qsearch fissi altrimenti)
...
static bool g_recapture_ext = false;   // Q-16 (Caissa): +2.05 Elo LOS 71.9% @2538g 15+0.15, ma LLR mai concluso e raddoppia i nodi (bench 377398->780972) -> evidenza troppo debole per il costo-albero, resta OFF. Riconsiderare dentro il batch C con conferma piu' solida.
// Filtro di TENSIONE (2026-07-12): estende solo se lo scambio e' ~pari (|SEE| <= soglia),
// non ogni ricattura. Uno scambio nettamente sbilanciato (es. donna-per-pedone) e' gia'
// ovvio, l'estensione li' e' spreco puro di nodi; il caso utile e' quando la posizione
// dopo la sequenza resta davvero incerta. g_recapture_tension in centipawn, UCI RecaptureTension.
...
int g_nc_bonus   = 2080;   // bonus max (quando la mossa ha assorbito TUTTI i nodi). Caissa: NodeCacheBonus 4096 [1000,8000]
int g_nc_min_sum = 1066;    // niente bonus finche' nodes_sum non supera questo (Caissa: 256)
...
int g_rfp_margin = 62;    // reverse futility: static_eval - g*depth >= beta   [SPSA-tuned: 30->21]
int g_razor_base = 293;   // razoring: base + mult*depth below alpha -> qsearch
int g_razor_mult = 77;   // [SPSA-tuned: 102->139]
int g_fut_base = 202;   // futility: base + mult*depth (+improving bonus)      [SPSA-tuned: 82->111]
...
int g_fut_improving = 186;    // extra futility margin when improving                [SPSA-tuned: 60->93]
...
// value(victim) + margin) but in the main search. SPSA targets = the cp MARGINS
...
int g_capfut_base  = 155;   // cp base margin
int g_capfut_mult  = 140;   // cp per reduced-depth ply (mirrors our quiet g_fut_mult scale)
int g_capfut_chist = 205;   // [F-002 audit 2026-07-02: 125->123, SPSA+SPRT] capture-history contribution, /1024
...
int  g_singular_tmargin = 319;        // triple-ext margin below singular_beta (> dmargin) (SPSA target)
// Negative extensions as CONTINUOUS tunable amounts (reach off=0). Default (1,0) = legacy
// (-1 on ttMove>=beta, nothing on cutNode). SF = (3,2). SPSA-tunable (small-int, calibrated step).
int  g_negext_tt  = 2;                // NegExtTT (SF=3): -extension when ttMove fails high over beta; 0=off, 1=legacy
int  g_negext_cut = 3;                // NegExtCut (SF=2): -extension on a cutNode (ttMove not fail-high); 0=legacy/off
static bool g_corrval_margin = true; // CorrValMargin (SF :980): fold |corr| into RFP margin (prune less when eval heavily corrected)
int  g_corrval_rfp = 41;              // |corr|*g/128 added to RFP margin (SPSA target)
// CorrValExt (5.0-B, audit SF-master/Reckless 2026-06-24, default OFF): estende il fold di |corr|
// OLTRE l'RFP -> anche futility, SEE, LMR (eval pesantemente corretta = incerta -> pota/riduci di MENO).
// Pesi co-tunabili. OFF = byte-identico.
...
int  g_doshallower_margin = 7;        // do_shallower se score < best + margin (SPSA target)
...
int  g_hindsight_margin = 3;          // [F-006 audit 2026-07-02: 3->4, SPRT +5.96 Elo LOS75% @408g] ply minimi
                                      // di riduzione perche' scatti l'hindsight (soglia piu' selettiva = meno
                                      // ri-estensioni sprecate)
// 5.1 HindsightRed (Stormphrax #300, complemento dell'extend): se un nodo non-PV e' stato RIDOTTO
// >= margine plies e l'eval e' girata BENE (static_eval + prev_eval >= soglia), RIDUCI (-1): la
// posizione regge nonostante la riduzione -> risparmia nodi. Default OFF (in test). Soglia in scala
// EvalScale-compressa (Stormphrax usa 202 in scala piena ~= 113 nostra).
static bool g_hindsight_red = false;
int  g_hindsight_red_margin = 2;      // ply minimi di riduzione (Stormphrax usa 2)
int  g_hindsight_red_thresh = 113;    // static_eval + prev_eval >= soglia (co-tunabile)
...
int  g_rfp_depth_cap = 7;             // RFPDepth: >0 overrides RFP depth cap (0=legacy 6/8); widen toward SF=17
...
// linea ~2803) NASCONDE il problema dell'ordinamento scadente; l'IID lo RISOLVE: senza TT move fa
// una mini-ricerca ridotta PRIMA per OTTENERE una hash move (ordinamento ~SF), cosi' il taglio
// "morde" (chicken-egg col cut-block). Solo su pv/cut node, con freno anti-cascata (no IID dentro IID).
// OFF = byte-identico (blocco saltato). Costa +nodi (la mini-ricerca) ma ordina molto meglio.
static bool g_iid = false;
int  g_iid_depth = 4;                  // IIDDepth: profondita' minima per attivare l'IID
int  g_iid_reduction = 2;              // IIDReduction: ply tolti alla mini-ricerca (iid_depth = depth - r)
// Q-15 FollowPV (SF PR #6656): si salva la PV dell'iterazione precedente; i nodi che
// la RISEGUONO (stessa linea di mosse dalla root) NON fanno IIR ne' shallow-pruning
// (futility/LMP): sulla linea principale l'ordinamento e' gia' buono e potare li'
// rischia di scartare la mossa migliore. Flag propagato lungo la sola PV-line.
// 0 = OFF (byte-identico). Usa td.prev_pv[] (salvata a fine iterazione).
...
int g_qs_delta          = 1488;  // F-002: delta-pruning qsearch (era const DELTA=1000; a scala-56 ~1785cp reali)
int g_singular_mpd      = 1;     // F-002: margine singular PER-DEPTH (era hardcoded 2*depth, unita'-eval)
...
bool g_see_ge = true;    // [F-008 2026-07-02: ON] +1.81% NPS su test rigoroso (150 pos x2, depth 15,
                         // nodi BIT-IDENTICI 73589060=73589060 -> solo velocita', zero rischio search)
...
int g_lmr_min_depth     = 3;     // F-005: depth minima per la LMR (SPSA: stabile a 3, confermato)
int g_lmr_min_moves     = 1;     // [F-005 BAKE 2026-07-02: 3->2, SPSA+SPRT in blocco LMR] riduce dalla 3a mossa (SF dalla 2a)
static bool g_lmr_captures = true;  // [F-004 BAKE 2026-07-02] LMR anche sulle CATTURE (SF/Stormphrax-style).
                                    // Vettore SPSA (MinMoves=2, EvalMargin=20) SPRT: +7.38 LOS 87% @800g 8+0.08
int g_lmr_cap_scale     = 52;    // F-004: % della lmr_table applicata alle catture (50 = dimezzata; SPSA confermo' ~50)
...
// iterazione; delta += (i64)|avgSq| * mult >> 20. Score grandi (verso il matto) ->
// finestra piu' larga -> meno ri-cerche. 0 = OFF (nessun termine, byte-identico).
// ⚠️ i64 obbligatorio: score*|score| ~9e8, *mult sfora i32 (overflow che Stormphrax
// ha dovuto fixare con un commit dedicato). NON accoppiare con AspAvg (revertito).
int g_asp_score_mult = 38;   // >> 20 scale; SPSA range [0,2000], init-when-ON ~100
...
// Q-20c (Caissa Search.cpp:1786): la futility NON pota mai la PRIMA quiet del nodo
// (quietIndex>1): almeno una quiet va sempre guardata, il segnale statico da solo
// non basta a chiudere il nodo. Toggle. OFF = comportamento storico.
static bool g_fut_spare_quiet = false;
// Q-20d (Ethereal search.c:872 + move.c:576): delta-pruning qsearch col BEST-CASE
// reale (vittima piu' grossa presente + potenziale promozione) invece del margine
// fisso: se nemmeno il colpo migliore possibile porta sopra alpha, il nodo e' morto.
...
int g_corr_cap = 50;    // max correction applied to static eval (cp). [SPSA histmulti 32->44 RIGETTATO: -1.3 LOS23% @11438 -> default]
int g_corr_lr_div = 303;   // learning-rate divisor (bigger = slower/steadier learning) [histmulti 512->565 rigettato]
...
int g_conthist_red_div = 3684;  // LMR: continuation-history reduction divisor [SPSA-tuned 5000->6595; histmulti 6595->7050 rigettato]
...
static bool g_cutnode_lmr   = true;    // CutNodeLMR: riduzione extra sui cut-node (SF li riduce di piu'). BAKED ON 2026-06-24 (2o segnale +: on+tuned = +11 @287g su Blackwell).
...
int g_threat_scale = 4212;    // contributo = scale/100 * pieceValue * (from_minacciato - to_minacciato). Spin ThreatScale (SPSA). Default basso = nudge (1500 = +75% nodi a d20 = disturba l'ordering tarato); il co-tune trova il valore nostro.
...
int g_check_bonus = 13357;    // bonus ordering per quiet che da scacco diretto (SF=16384). Spin CheckBonus (SPSA).
...
int g_prior_bonus_scale = 189;   // /100 del td_stat_bonus(depth). Spin PriorBonusScale.
...
int g_lowply_weight = 179;    // contributo ordering = g_lowply_weight * lowply / (100*(1+2*ply)) (SF-style decay). Spin LowPlyWeight.
...
int  g_ttcut_fifty           = 89;     //    soglia fifty oltre cui il cutoff TT non e' affidabile (patta-50 vicina) [BAKE 2026-07-04 blocco secondaudit_block SPSA, SPRT +8.34 Elo LOS79.4% @500g (non conclusivo ma leaning+)]
// Q-14 TTResearchMargin (Ethereal, tecnica di Grant): fail-low anticipato a depth-1.
// Entry UPPER (hash_flag_alpha) a tt_depth>=depth-1 -> return se !PV && (cutnode ||
// ttScore<=alpha) && ttScore+margine <= alpha. Duale del TTCutRefine (+1ply ai
// fail-high). Margine 141 Ethereal ~= 79 alla nostra scala-56 (init SPSA). Default OFF.
...
int  g_negext_alpha          = 1;      // #6d negext se ttScore <= alpha (0=off; Berserk 1) [BAKE 2026-07-04 blocco secondaudit_block SPSA, SPRT +8.34 Elo LOS79.4% @500g (non conclusivo ma leaning+)]
...
int  g_fhboost_margin        = 2;      // #10a bonus history a depth+1 se best > beta+margine (Obsidian 95, Berserk 75). Bake 2026-07-07 (margin=50) REVERTITO: i "conferma" 2x avevano AspAvg ON; retest ISOLATO vs baseline pura = +1.74 NULLO. OFF (0), re-ispezione a TC lungo
...
static bool g_killer_reset   = true;  // #13 azzera i killer del ply FIGLIO a ogni nodo (anti-stale, +12.15 Elo)
...
int g_lmr_ss_div    = 5430; // [4.1: tenuto 4.0 - il BAKE @12s 13790 GONFIAVA l'albero 2.5x (meno riduzione) per ~pochi Elo: scelta = albero stretto > Elo]
int g_lmr_ss_offset = -2953;  // [4.1: tenuto 4.0 - vedi sopra; il BAKE -2214 = parte del bloat LMR, revertito]
int g_lmr_ch_div    = 10942;   // [3.7 BAKE 4437->3506; BAKED #1 era 10000]. ContHistLMR: reduction -= (conthist1+2+4) / div
...
int g_nmp_div = 3;
int g_lmr_eval_margin = 43;   // [F-004/005 BAKE 2026-07-02: 33->20, SPSA in blocco LMR] LMR: reduce +1 more when static_eval + margin < alpha
int g_lmr_ttdepth = 1;     // LMR: reduce LESS by this when TT depth >= depth   [SPSA-tuned: 0->2]
// CORE LMR formula coefficients (*100).
int g_lmr_base_x100 = 22;    // baseline reduction floor [3.7 BAKE 41->37; SPSA 75->47; BAKED #1 47->41]
int g_lmr_div_x100 = 447;   // bigger divisor = LESS reduction [3.7 BAKE 345->310; SPSA 225->270; BAKED #1 270->345]
...
// SF (search.cpp:1287-1334) la monta in millesimi di ply fondendo ~12 segnali con peso fine, e
...
int g_lmrf_cut      = 3687;   // [5.1 BAKE 3995->3184] cut-node: riduzione forte (il divario #1)
int g_lmrf_cut_nott = 2397;   // [5.1 BAKE 1059->2092] extra sui cut-node senza TT move
int g_lmrf_ttcap    = 200;    // [5.1 BAKE 1039->390]  ttMove e' una cattura -> riduci di piu'
int g_lmrf_ttpv     = 617;   // [5.1 BAKE 2766->2210] ttPv: riduci MENO (protezione del ramo ex-PV)
int g_lmrf_pv       = 437;   // [5.1 BAKE 1017->1551] PV node: riduci MENO
int g_lmrf_ss       = 664;    // [5.1 BAKE 445->923]   statScore (history continua): r -= statScore*g/4096
int g_lmrf_corr     = 866;    // [5.1 BAKE 160->478]   correctionValue (eval incerta -> riduci meno)
int g_lmrf_all      = 618;    // [5.1 BAKE 272->723]   scaling ALL-node: r += r*g/(256*depth+285)  [ci mancava]
...
int g_histprune_margin = 2097;  // [3.7 BAKE 1602->1691; BAKED #1 era 1000]. history pruning: prune late quiet if combined hist < -margin*depth
int g_conthist_prune_depth = 2; // gate profondita' conthist-prune (SF usa lmrDepth<6). UCI ContHistPruneDepth. PASSO2: col blocco LmrDepthPrune si alza ~6.
...
// PASSO 1 (SF Step-14): protezione-history. Con LmrDepthPrune ON, prune_depth viene
// AGGIUSTATO dalla history della mossa: prune_depth += history/g_lmrdepth_histdiv.
// History buona -> prune_depth sale (mossa protetta); scarsa -> scende (potata di piu').
// E' il pezzo che mancava al port monco (potava le tardive alla cieca -> bloat). SF div=6437.
...
bool set_search_param(const char* name, int value) {
    ...
    if (!strcmp(name, "FollowPV"))            { g_follow_pv = value != 0; return true; }
    if (!strcmp(name, "RFPBadNode"))          { g_rfp_badnode = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "FutSpareQuiet"))       { g_fut_spare_quiet = value != 0; return true; }
    if (!strcmp(name, "QSDeltaBestCase"))     { g_qs_delta_bestcase = value != 0; return true; }
    if (!strcmp(name, "QSBCMargin"))          { g_qs_bc_margin = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "LmrDepthHistDiv"))     { g_lmrdepth_histdiv = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "ContHistPruneDepth"))  { g_conthist_prune_depth = value < 1 ? 1 : value; return true; }
    ...
    if (!strcmp(name, "CaptureFutility"))     { g_cap_futility = value != 0; return true; }
    if (!strcmp(name, "CapFutBase"))          { g_capfut_base  = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "SingularTripleMargin")){ g_singular_tmargin = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "NegExtTT"))            { g_negext_tt  = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "NegExtCut"))           { g_negext_cut = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "CorrValMargin"))       { g_corrval_margin = value != 0; return true; }
    if (!strcmp(name, "CorrValRFP"))          { g_corrval_rfp = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "CorrValExt"))          { g_corrval_ext = value != 0; return true; }
    ...
    if (!strcmp(name, "HindsightRed"))        { g_hindsight_red = value != 0; return true; }
    if (!strcmp(name, "HindsightRedMargin"))  { g_hindsight_red_margin = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "HindsightRedThresh"))  { g_hindsight_red_thresh = value; return true; }
    ...
    if (!strcmp(name, "LMREnrich"))           { g_lmr_enrich = value != 0; return true; }
    if (!strcmp(name, "LMREnrichAmount"))     { g_lmr_enrich_amount = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "RazorQuadCoef"))       { g_razor_quad_coef = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "RFPDepth"))            { g_rfp_depth_cap = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "RazorDepth"))          { g_razor_depth_cap = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "IID"))                 { g_iid = value != 0; return true; }
    if (!strcmp(name, "IIDDepth"))            { g_iid_depth = value < 2 ? 2 : value; return true; }
    if (!strcmp(name, "IIDReduction"))        { g_iid_reduction = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "LMRFine"))             { g_lmr_fine = value != 0; return true; }
    if (!strcmp(name, "LMRFCut"))             { g_lmrf_cut = value; return true; }
    if (!strcmp(name, "LMRFCutNoTT"))         { g_lmrf_cut_nott = value; return true; }
    if (!strcmp(name, "LMRFTTCap"))           { g_lmrf_ttcap = value; return true; }
    if (!strcmp(name, "LMRFTTPv"))            { g_lmrf_ttpv = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "LMRFPv"))              { g_lmrf_pv = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "LMRFSS"))              { g_lmrf_ss = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "LMRFCorr"))            { g_lmrf_corr = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "LMRFAll"))             { g_lmrf_all = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "SeeGE"))               { g_see_ge = (value != 0); return true; }
    if (!strcmp(name, "SeeGEVerify"))         { g_see_ge_verify = (value != 0); if (value) { g_seege_checks = 0; g_seege_bad = 0; } else seege_verify_report(); return true; }
    if (!strcmp(name, "QSDeltaMargin"))       { g_qs_delta = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "SingularMarginPD"))    { g_singular_mpd = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "TMDropThresh"))        { g_tm_drop_thresh = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "TMDropCap"))           { g_tm_drop_cap = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "NodeTMBase"))          { g_nodetm_base = value < 100 ? 100 : value; return true; }
    if (!strcmp(name, "NodeTMMin"))           { g_nodetm_min = value < 10 ? 10 : (value > 100 ? 100 : value); return true; }
    if (!strcmp(name, "LMRMinDepth"))         { g_lmr_min_depth = value < 2 ? 2 : value; return true; }
    if (!strcmp(name, "LMRMinMoves"))         { g_lmr_min_moves = value < 1 ? 1 : value; return true; }
    if (!strcmp(name, "LMRCaptures"))         { g_lmr_captures = value != 0; return true; }
    if (!strcmp(name, "LMRCapScale"))         { g_lmr_cap_scale = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "TTSecondaryAge"))      { g_tt_secondary_age = value != 0; return true; }
    ...
    if (!strcmp(name, "RecaptureTension"))    { g_recapture_tension = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "TTPvAmount"))          { g_ttpv_amount = value < 0 ? 0 : (value > 2 ? 2 : value); return true; }
    if (!strcmp(name, "ThreatScale"))         { g_threat_scale = value < 0 ? 0 : value; return true; }
    if (!strcmp(name, "ThreatHistWeight"))    { g_threat_hist_weight = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "TTResearch"))          { g_tt_research = value != 0; return true; }
    if (!strcmp(name, "TTResearchMargin"))    { g_tt_research_margin = value < 0 ? 0 : value; return true; }
    ...
    if (!strcmp(name, "TTMoveKeep"))          { g_tt_move_keep = value != 0; return true; }
...
static inline int td_see_at_least(ThreadData& td, int move, int thr) {
    if (g_see_ge_verify) {
        ...
        if (ge != cl) {
            ...
            printf("info string SeeGE MISMATCH #%lld move=%d thr=%d ge=%d classic=%d (checks=%lld)\n",
                   b, move, thr, ge, cl, (long long)g_seege_checks);
            fflush(stdout);
        }
        return cl;
    }
    return g_see_ge ? td_see_ge(td, move, thr) : ((td_see(td, move) >= thr) ? 1 : 0);
}
...
static inline void td_compute_threats(ThreadData& td) {
    ...
    // threat_all = byR + regina + re (TUTTE le minacce nemiche) per la ThreatHist. Calcolato
    // sempre qui (cheap); letto solo quando g_threat_hist e' on.
    U64 byAll = byR;
    bb = td.bitboards[base + Q];           // enemy queens
    while (bb) { int s = get_ls1b_index(bb); pop_bit(bb, s); byAll |= get_queen_attacks(s, occ); }
...
// tier-minaccia di una casa (VALUE-AWARE, cheapest-attacker): 0 = safe, 1 = attaccata da pawn/knight/
// bishop (attaccante economico -> pericoloso per pezzi di valore), 2 = solo da rook/queen/king. Usa i
// tier che gia' calcoliamo (threat_by_minor = pawn+minori, threat_all = tutte).
static inline int td_sq_tier(ThreadData& td, int sq) {
    if (get_bit(td.threat_by_minor, sq)) return 1;
    if (get_bit(td.threat_all, sq))      return 2;
    return 0;
}
// ThreatHist REPLACE-TIERED: bucket-minaccia di una mossa = from_tier*3 + to_tier (0..8). Ritorna 0
// quando ThreatHist e' OFF -> history_moves[0][..] = vecchia tabella = byte-identico. Da chiamare col
// board del nodo CORRENTE (i tier = minacce di QUESTO nodo).
static inline int td_hbucket(ThreadData& td, int move) {
    if (!g_threat_hist) return 0;
...
static inline int td_score_move(ThreadData& td, int move, int tt_move) {
    ...
    // (ThreatHist REPLACE: la threat-indicizzazione e' GIA' dentro history_moves[bucket][..] sopra,
...
static int td_quiescence(ThreadData& td, int alpha, int beta, int qs_depth = 0) {
    ...
    td.nodes++;
    if (g_node_limit && td.nodes >= g_node_limit) { stop_threads.store(true, std::memory_order_relaxed); return 0; }
    ...
    if (in_check) {
        ...
        // Q-20d (Ethereal): delta-pruning col BEST-CASE reale — vittima piu' grossa
        // sul board + potenziale promozione (pedone in 7a relativa). Se nemmeno il
        // colpo migliore possibile + cuscino arriva ad alpha, il nodo e' morto.
        if (g_qs_delta_bestcase) {
            int maxvic;
            if (td.side == white) {
                maxvic = td.bitboards[q] ? 900 : td.bitboards[r] ? 500
                       : (td.bitboards[b] | td.bitboards[n]) ? 330
                       : td.bitboards[p] ? 100 : 0;
                if (td.bitboards[P] & 0xFF00ULL) maxvic += 800;              // promo: +Q-P
            } else {
                maxvic = td.bitboards[Q] ? 900 : td.bitboards[R] ? 500
                       : (td.bitboards[B] | td.bitboards[N]) ? 330
                       : td.bitboards[P] ? 100 : 0;
    ...
    for (int count = 0; count < move_list->count; count++) {
        ...
        if (!in_check) {
            if (!get_move_capture(move)) {
                ...
                if (!td_see_at_least(td, move, -75)) continue;
            } else {
                ...
                if (g_bad_noisy && !get_move_promoted(move) && !get_move_enpassant(move)) {
                    int prev_bn = td.move_stack[td.ply];
                    int prev_bn_sq = prev_bn ? get_move_target(prev_bn) : -1;
                    if (get_move_target(move) != prev_bn_sq && qcaptures >= g_bad_noisy_count)
                        continue;
                }
                qcaptures++;
                // (4) QFutility: salta la cattura se best_score + valore(vittima) + margine
                //     non arriva ad alpha (no ep/promo). OFF = nessun effetto.
                //     FIX SCALA (2026-06-24): best_score/alpha sono eval-NET (~3x la scala-100;
                //     pedone marginale misurato ~190-500cp via `eval`), mentre see_piece_values e'
                //     scala-100 -> il termine vittima va scalato x3, altrimenti la qsearch sovra-pota
                //     (era -126 Elo). x3 SOVRASTIMA i pezzi grossi (l'eval-net satura) = errore nella
                //     direzione SICURA (pota MENO le catture grosse). QFutMargin (eval-scale) tara il residuo.
                if (g_qfutility && !get_move_promoted(move) && !get_move_enpassant(move)) {
                    ...
                    // (to_sq != prevSq) — potarla = perdere la valutazione del recupero (era il −17).
                    ...
                    if (vic >= 0 && vic < 12 && get_move_target(move) != prev_sq) {
                        ...
                        if (fv <= alpha) {
                            if (fv > best_score) best_score = fv;   // SF fail-soft: bestValue = max(bestValue, fv)
                            continue;
                        }
                    }
                }
                if (!get_move_promoted(move) && !td_see_at_least(td, move, 0)) continue;
            }
        }
...
// Q-12 CMHC (SF PR #6653): fattore di scala % del bonus CONTHIST per la CONSISTENZA
// della cutoff-move nei contesti della continuation-stack. Piu' contesti in cui la
// mossa ha gia' continuation-history POSITIVA -> update piu' grande (rinforza le mosse
// buone-in-piu'-contesti). Usa gli STESSI ply/tabelle che l'update tocca (1-ply sempre,
// 2/4-ply e 3/6-ply sotto i rispettivi toggle). 0 = neutro (fattore 100, byte-identico).
static inline int td_cmhc_factor(ThreadData& td, int move) {
    if (!g_cmhc_scale) return 100;
    int pc = get_move_piece(move), tg = get_move_target(move);
    int consistent = 0;
    if (td.ply >= 1) { int p = td.move_stack[td.ply - 1];
        if (p && td.continuation_history[get_move_piece(p)][get_move_target(p)][pc][tg] > 0) consistent++; }
    if (g_conthist_multi) {
        if (td.ply >= 1) { int p = td.move_stack[td.ply - 1];
            if (p && td.cont_hist_2[get_move_piece(p)][get_move_target(p)][pc][tg] > 0) consistent++; }
        if (td.ply >= 3) { int p = td.move_stack[td.ply - 3];
            if (p && td.cont_hist_4[get_move_piece(p)][get_move_target(p)][pc][tg] > 0) consistent++; }
    }
    if (g_conthist36) {
        if (td.ply >= 2) { int p = td.move_stack[td.ply - 2];
            if (p && td.cont_hist_3[get_move_piece(p)][get_move_target(p)][pc][tg] > 0) consistent++; }
        if (td.ply >= 5) { int p = td.move_stack[td.ply - 5];
            if (p && td.cont_hist_6[get_move_piece(p)][get_move_target(p)][pc][tg] > 0) consistent++; }
    }
    return 100 + g_cmhc_scale * consistent;
}

int td_negamax(ThreadData& td, int alpha, int beta, int depth, bool is_cut_node, int excluded_move = 0, bool follow_pv = false) {
    ...
    if (tt_hit && td.ply && !excluded_move) {
        ...
        // Q-14 TTResearchMargin (Ethereal): fail-low anticipato a depth-1. Solo entry
        // UPPER a tt_depth>=depth-1, !PV, e ttScore ben sotto alpha (margine). Non si
        // sovrappone al cutoff principale sopra: se tt_depth>=depth e alpha-bound cuttava,
        // il return e' gia' avvenuto. Adjust matto locale (l'entry e' relativa alla root).
        if (g_tt_research && !pv_node && tt_depth >= depth - 1
            && tt_flag == hash_flag_alpha) {
            int adj = tt_score;
            if (adj < -mate_score) adj += td.ply;
            if (adj >  mate_score) adj -= td.ply;
    ...
    // mini-ricerca ridotta PRIMA per OTTENERE una hash move buona -> ordinamento ~SF
    // -> il taglio "morde". Solo su pv/cut node (dove l'ordinamento conta), con freno
    // anti-cascata thread_local (niente IID dentro la mini-ricerca: eviterebbe la
    // ricorsione esplosiva). OFF (g_iid=false) = blocco saltato = byte-identico.
        ...
        && (pv_node || is_cut_node) && depth >= g_iid_depth)
    {
        ...
        if (iid_depth >= 1)
        {
            ...
            tl_in_iid = false;
            // Ri-probe la TT per la mossa appena trovata (solo tt_move; il resto in scratch
            // -> non tocchiamo tt_hit/tt_score/tt_depth/tt_flag: l'IID serve all'ORDINAMENTO).
            int s2 = 0, d2 = 0, f2 = hash_flag_alpha, e2 = tt_eval_none; bool p2 = false;
    ...
    // shave the RFP margin (prune a touch more). Gated like improving. Off = no effect.
    bool opp_worsening = false;
    if (g_opp_worsening && !in_check && td.ply >= 1) {
        int e1 = td.eval_stack[td.ply - 1];
        if (e1 != EVAL_NONE) opp_worsening = static_eval > -e1;
    }
    ...
    if (g_seo_mult && !in_check && td.ply >= 1) {
        ...
        if (prev_move != 0 && td.captured_stack[td.ply] == -1
            ...
            td_update_history(td.history_moves[td.hbucket_stack[td.ply]][pp][pt], bonus);
    ...
    if (g_hindsight_ext && !pv_node && !excluded_move && !in_check && td.ply >= 1
        ...
        depth++;
    // 5.1 HindsightRed (Stormphrax #300, complemento): nodo ridotto >= margine ma eval girata BENE
    // (static_eval + prev_eval >= soglia) -> RIDUCI (-1): la posizione regge, risparmia nodi.
    // else-if -> mutuamente esclusivo con l'extend (sum<0 vs sum>=soglia). depth>=2 per non azzerare.
    else if (g_hindsight_red && !pv_node && !excluded_move && !in_check && depth >= 2 && td.ply >= 1
        && td.eval_stack[td.ply - 1] != EVAL_NONE
    ...
    while ((move = mp_next(td, mp)) != 0) {
        ...
        if (g_lmrdepth_prune && is_quiet && depth >= 3 && moves_searched >= 1) {
            ...
            // PASSO 1 (SF): protezione-history. prune_depth += history/div -> le mosse con
            // buona storia (probabili cutoff) salgono e NON vengono potate; le scarse scendono.
            // SF: history = 2*mainHist + contHist + pawnHist. E' cio' che mancava al port monco.
            int prev_m = td.move_stack[td.ply];
            int hist = 2 * td.history_moves[td_hbucket(td, move)][get_move_piece(move)][get_move_target(move)];
            if (prev_m)
                hist += td.continuation_history[get_move_piece(prev_m)][get_move_target(prev_m)]
        ...
        // LMP  (Q-15: mai sui nodi che seguono la PV — l'ordinamento li' e' gia' fidato)
        if (!pv_node && !in_check && is_quiet && best_score > -mate_score && !follow_pv) {
        ...
        if (!pv_node && !in_check && prune_depth <= g_fut_depth && is_quiet && best_score > -mate_score && !follow_pv
        ...
        if (g_cap_futility && !pv_node && !in_check && is_capture && !is_promotion &&
            ...
            int d_idx = depth < 64 ? depth : 63;
            int m_idx = moves_searched < 64 ? moves_searched : 63;
            int cap_lmr_depth = depth - 1 - lmr_table[d_idx][m_idx];   // reduced-depth estimate
            if (cap_lmr_depth < 0) cap_lmr_depth = 0;
        ...
        if (g_cont_hist_prune && !pv_node && !in_check && is_quiet &&
            prune_depth <= g_conthist_prune_depth && moves_searched > 0 && best_score > -mate_score) {
            int prev = td.move_stack[td.ply];
            int hh = td.history_moves[td_hbucket(td, move)][get_move_piece(move)][get_move_target(move)];
            if (prev)
        ...
        if (excluded_move == 0 && move == tt_move && depth >= g_singular_mindepth && td.ply &&
            ...
            // high over beta -> shrink by g_negext_tt (default 1=legacy, SF=3); else on a
            // cut node -> shrink by g_negext_cut (default 0=off, SF=2). Co-tunable -> SPSA
            // can dial a harmful negative extension back to 0.
        ...
        // Filtro TENSIONE (2026-07-12): estende solo se |SEE(ricattura)| <= soglia, cioe'
        // lo scambio e' ~pari. Uno scambio nettamente sbilanciato e' gia' ovvio (SEE alto
        // in un verso o nell'altro) -> l'estensione li' non compra nulla, costa solo nodi.
        ...
        // ThreatHist: bucket-minaccia di QUESTA mossa nel contesto del nodo PADRE (board corrente,
        // PRE-make) -> usato dal read LMR (post-make) e messo nello stack per seo/prior del figlio.
        ...
        bool child_follow = g_follow_pv && follow_pv
                         && td.ply < td.prev_pv_len && move == td.prev_pv[td.ply];
        ...
        if (moves_searched == 0) {
            score = -td_negamax(td, -beta, -alpha, depth - 1 + extension, false, 0, child_follow);
        }
        else {
            int reduction = 0;

            // F-005: gate LMR promossi a tunable (erano hardcoded 3/3; SF riduce da depth>=2, 2a mossa).
            if (depth >= g_lmr_min_depth && moves_searched >= g_lmr_min_moves && is_quiet && !in_check && !gives_check) {
              ...
              if (!g_lmr_fine) {
                ...
                if (g_ttpv_amount > 0 && tt_hit && tt_pv && !pv_node) reduction -= g_ttpv_amount;
                ...
                // LMREnrich (archivio 4.2, default OFF): TT-move noisy + mossa corrente quiet
                // -> riduci di piu' (SF ttCapture). OFF = byte-identico.
                if (g_lmr_enrich && tt_move &&
                    (get_move_capture(tt_move) || get_move_enpassant(tt_move) || get_move_promoted(tt_move)))
                ...
                int raw_hist = td.history_moves[move_hbucket][get_move_piece(move)][get_move_target(move)];
                if (g_statscore_lmr) {
                ...
                // CorrValExt: eval pesantemente corretta = incerta -> riduci di MENO (come SF/Reckless).
                if (g_corrval_ext) reduction -= (corr_val < 0 ? -corr_val : corr_val) * g_corrval_lmr / 4096;
              } else {
                // ⭐ Riduzione FINE stile-SF in 1/1024 di ply (port search.cpp:1287-1334): blenda i
                // segnali con peso fine e taglia FORTE solo dove e' sicuro. r in millesimi di ply.
                long long r = (long long)lmr_table[d_idx][m_idx] * 1024;   // base dalla nostra tabella
                if (tt_pv && !pv_node) r -= g_lmrf_ttpv;                   // protezione ex-PV
                if (pv_node)           r -= g_lmrf_pv;                     // protezione PV
                r -= (long long)(corr_val < 0 ? -corr_val : corr_val) * g_lmrf_corr / 4096;  // eval incerta -> meno
                if (is_cut_node) r += g_lmrf_cut + (tt_move == 0 ? g_lmrf_cut_nott : 0);      // CUT-NODE forte (il divario #1)
                if (tt_move && (get_move_capture(tt_move) || get_move_enpassant(tt_move) || get_move_promoted(tt_move)))
                    r += g_lmrf_ttcap;
                // statScore continuo (2*main + cont1 + cont2), come SF
                int sscore = 2 * td.history_moves[move_hbucket][get_move_piece(move)][get_move_target(move)];
                { int prv = td.move_stack[td.ply - 1];
                  if (prv) sscore += td.continuation_history[get_move_piece(prv)][get_move_target(prv)][get_move_piece(move)][get_move_target(move)];
                  if (td.ply >= 2) { int p2 = td.move_stack[td.ply - 2];
            ...
            // F-004 (audit 2026-07-02): LMR anche sulle CATTURE (SF/Stormphrax riducono le
            // noisy tardive; noi mai = gap strutturale). Riduzione dalla stessa lmr_table
            // scalata % (default 50 = dimezzata), MAI su promozioni/scacchi, con esenzione
            // RICATTURA sulla casa appena mossa dall'avversario (stessa esenzione della
            // QFutility: potare/ridurre il recupero = perdere la valutazione dello scambio).
            // Default OFF = byte-identico.
            else if (g_lmr_captures && is_capture && !is_promotion && !in_check && !gives_check
                     && depth >= g_lmr_min_depth && moves_searched >= g_lmr_min_moves) {
            ...
            score = -td_negamax(td, -alpha - 1, -alpha, reduced_depth, true, 0, child_follow);
            ...
            if (score > alpha && reduction > 0) {
                ...
                if (g_do_deeper) {
                    bool do_deeper    = score > best_score + g_dodeeper_base + 2 * full_depth;
                ...
                if (full_depth > reduced_depth) {
                    score = -td_negamax(td, -alpha - 1, -alpha, full_depth, !is_cut_node, 0, child_follow);
            ...
            if (score > alpha && score < beta) {
                score = -td_negamax(td, -beta, -alpha, full_depth, false, 0, child_follow);
            }
        }
            ...
            if (score > alpha) {
                ...
                if (score >= beta) {
                    ...
                    if (g_cutoff_stats) {
                        ...
                        if (tt_move) {
                            td.fh_tt++;
                            if (moves_searched == 1) td.fh_tt_first++;
                        }
                        if (tt_hit) td.fh_probe++;   // entry TT presente (anche senza mossa)
                    }
                    ...
                    if (is_quiet) {
                        ...
                        // Q-12 CMHC: il bonus CONTHIST (non la main-history) e' scalato per
                        // la consistenza della mossa nei contesti conthist. 0 = neutro.
                        int chbonus = bonus * td_cmhc_factor(td, move) / 100;
                        if (prev_cm)
                            td_update_history(td.continuation_history[pcp][pct][get_move_piece(move)][get_move_target(move)], chbonus);
                        ...
                        for (int q = 0; q < n_searched_quiets; q++) {
                            ...
                            if (g_pawn_hist)
                                td_update_history(td.pawn_history[pawn_pk][get_move_piece(qm)][get_move_target(qm)], -qmalus);
                            if (g_lowply && td.ply < ThreadData::LOW_PLY_MAX)
                                td_update_history(td.lowply_history[td.ply][get_move_piece(qm)][get_move_target(qm)], -qmalus);
                        }
                    }
    ...
    if (g_prior_bonus && hash_flag == hash_flag_alpha) {
        ...
        if (mv1) {
            ...
            if (!get_move_capture(mv1)) {
                td_update_history(td.history_moves[td.hbucket_stack[td.ply]][p1][t1], pbonus);
                if (td.ply >= 1) {
...
static void thread_search(int thread_id, int max_depth) {
    ...
    long long avg_sq_score = 0;      // Q-28 AspScoreMult: EMA di score*|score| (valida sse avg_score != infinity)
    td.prev_pv_len = 0;              // Q-15: nessuna PV precedente alla prima iterazione di questa search
    ...
    for (int current_depth = start_depth; current_depth <= max_depth; current_depth++) {
        ...
        while (true) {
            ...
            score = td_negamax(td, alpha, beta, search_depth, false, 0, g_follow_pv);   // Q-15: la root segue sempre la PV
        ...
        prev_score = score;
        // Q-28: aggiorna l'EMA quadratica PRIMA di avg_score (usa avg_score come flag first-time)
        {
        ...
        if (td.pv_length[0] > 0) {
            ...
            if (g_root_mate_restore && td.depth > 0 && prev_abs >= mate_score
                ...
                if (g_follow_pv) {
                    ...
                    if (len > max_ply) len = max_ply;
                    for (int i = 0; i < len; i++) td.prev_pv[i] = td.pv_table[0][i];
                    td.prev_pv_len = len;
                }
        ...
        if (thread_id == 0 && timeset) {
            ...
            if (g_time_mgmt && current_depth > start_depth) {
                ...
                if (drop > g_tm_drop_thresh) {             // F-002: era hardcoded 8/200 (unita'-eval)
            ...
            if (g_node_tm && current_depth >= 6 && iter_nodes > 0) {
                double frac = (double)td.root_bestmove_nodes / (double)iter_nodes;
                if (frac > 1.0) frac = 1.0;
                double scale = g_nodetm_base / 100.0 - frac;   // F-002: era hardcoded 1.4/0.6
...
void search_position_mt(int depth) {
    ...
    if (g_cutoff_stats) {
        ...
        if (fh) {
            ...
            printf("info string FMC %.2f%% (fh=%llu first=%llu) avgidx %.3f | ttrate %.2f%% probehit %.2f%% FMC|tt %.2f%% FMC|nott %.2f%%\n",
