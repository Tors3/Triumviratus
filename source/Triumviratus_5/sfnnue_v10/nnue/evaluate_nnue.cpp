...
bool read_parameters(std::istream& stream, T& reference) {
    ...
    std::uint32_t header;
    header = read_little_endian<std::uint32_t>(stream);  // u32 component hash prefix
    (void) header;  // SFNNv10 nn-49c: structure validated via NNUE_VERIFY, not hash
    if (!stream)
        return false;
...
static bool read_header(std::istream& stream, std::uint32_t* hashValue, std::string* desc) {
    ...
    *hashValue = read_little_endian<std::uint32_t>(stream);
    ...
    // Don't gate on version: master nnue-pytorch 89d5725 uses 0x6A448AFA, SF18
    // uses 0x7AF32F20. Structure is validated by the FT/layer-stack reads.
    (void) version;
    if (!stream)
        return false;
    desc->resize(size);
...
static bool read_parameters(std::istream& stream, NetSize netSize, std::string& netDescription) {
    ...
    if (!read_header(stream, &hashValue, &netDescription))
        return false;
    (void) hashValue;  // SFNNv10 nn-49c: don't gate on the net-level hash
...
// Raw eval: writes the int32 pre-blend psqt and positional values (Big net only).
// Used for bit-exact cross-validation against the Python reference (master
// nnue-pytorch). Skips ALL wrapper math (blend / complexity / optimism /
// rule50 / clamp). Pre-divide by OutputScale is NOT applied so the caller sees
// the same integers as the internal computation.
void evaluate_raw_big(const Position& pos, AccumulatorCaches* caches,
                      std::int32_t* psqtOut, std::int32_t* positionalOut) {
    constexpr uint64_t alignment = CacheLineSize;
    alignas(alignment) TransformedFeatureType
      transformedFeatures[FeatureTransformer<TransformedFeatureDimensionsBig, nullptr>::BufferSize];
    ASSERT_ALIGNED(transformedFeatures, alignment);
    const int bucket   = (pos.count<ALL_PIECES>() - 1) / 4;
    const auto psqt    = featureTransformerBig->transform(pos, transformedFeatures, bucket,
                                                          caches ? &caches->big : nullptr);
    const auto pos_out = networkBig[bucket]->propagate(transformedFeatures);
    if (psqtOut)       *psqtOut       = psqt;
    if (positionalOut) *positionalOut = pos_out;
}
template Value
