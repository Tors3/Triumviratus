...
function Build-Variant([string]$tag) {
    ...
    $noPersp = $false
    $baseTag = $tag
    # 25/09/2026: persp accesa anche sui target `-intel` (universalita', decisione utente).
    if ($tag -match '-intel$') { $baseTag = $tag -replace '-intel$','' }
    ...
    # ✅ 25/09/2026: persp ACCESA su tutti i target AVX-512 (misura a coppie simultanee su
    # Xeon Gold 6138: +0,80%, IC95 [+0,50, +1,10]). Resta spenta solo sui target `-intel`.
